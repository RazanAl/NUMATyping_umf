...
je_mallocx(size_t size, int flags) {
    ...
    if (first_time) {
        printf( "\033[1;31m Using Cloned Jemalloc!\033[0m\n");
        first_time = 0;
    }
	void *ret;
	static_opts_t sopts;
...
je_mallocv(size_t size, int flags) {
    ...
    if (first_time) {
        printf( "\033[1;31m Using Jemallocx \033[0m\n");
        first_time = 0;
    }
	LOG("core.mallocx.entry", "size: %zu, flags: %d", size, flags);

	void * ret = imallocx_fastpath(size, flags, &je_mallocx);

	LOG("core.mallocx.exit", "result: %p", ret);
	return ret;
}
/*
 * End mallocv functions.
 */
/******************************************************************************/
static void *
